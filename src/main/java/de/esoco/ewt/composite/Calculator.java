...
public class Calculator extends Composite
{
				   ...
				   BiFunction<BigDecimal, BigDecimal, BigDecimal>
	{
		...
		AND("And",
			...
			'&',
			(d1, d2) ->
			new BigDecimal(d1.toBigInteger()
						   .and(d2.toBigInteger()))),
		OR("Or",
		   1,
		   '|',
		   (d1, d2) -> new BigDecimal(d1.toBigInteger().or(d2.toBigInteger()))),
		XOR("Xor",
			1,
			'^',
			(d1, d2) ->
		...
		LEFT_SHIFT("<<",
				   ...
				   (d1, d2) ->
				   new BigDecimal(d1.toBigInteger()
								  .shiftLeft(d2.intValue()))),
		RIGHT_SHIFT(">>",
					1,
					'>',
					(d1, d2) ->
					new BigDecimal(d1.toBigInteger()
								   .shiftRight(d2.intValue())));
	...
	class CalculatorState
	{
		...
		void input(int nDigit)
		{
			...
			if (bFractionInput)
			{
				...
				dCurrentValue = dCurrentValue.add(dInputDigit.multiply(aDigit));
			}
			else
			{
