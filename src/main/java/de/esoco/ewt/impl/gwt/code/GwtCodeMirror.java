	...
	implements Focusable, IsTextArea, HasValueChangeHandlers<String>
{
	...
	public EditorPosition getCaretPosition()
	{
		return aCodeMirror != null
			   ? EditorPosition.fromJavaScriptObject(getEditorPosition(aCodeMirror))
	...
	public int getIndexFromEditorPosition(EditorPosition rPosition)
	{
		return aCodeMirror != null
			   ? calcIndexFromPosition(aCodeMirror,
									   rPosition.toJavaScriptObject()) : 0;
	}
	...
	public void setCursorPos(int nPosition)
	{
		JavaScriptObject rPosition =
			aCodeMirror != null ? calcPositionFromIndex(aCodeMirror, nPosition)
								: null;
	...
	public void setErrorRange(EditorPosition rStart, EditorPosition rEnd)
	{
		...
		JavaScriptObject rMark =
			markText(aCodeMirror,
					 rStart.toJavaScriptObject(),
					 rEnd.toJavaScriptObject(),
					 "error");
	 ...
	 *                     be added.
	 */
	private native void addElement(
		JavaScriptObject jsListObject,
	 ...
	 * @return The index for the editor position
	 */
	private native int calcIndexFromPosition(
		...
		AutoCompletionChoice rChoice)
	{
		...
		return createAutoCompletionResult(rChoice.getText(),
										  rChoice.getDisplayText(),
										  rChoice.getCssClassName(),
										  from,
										  to);
	}
	 ...
	 */
	private native JavaScriptObject createAutoCompletionResult(
		String			 text,
		String			 displayText,
		String			 className,
		JavaScriptObject from,
	...
	private native void doAutoCompleteCallback(
		JavaScriptObject rCallbackFunction,
		JavaScriptObject rArgument,
		int				 nLine,
		int				 nIndex) /*-{
		rCallbackFunction({
			list: rArgument,
	 ...
	 *                        done by calling {@link
	 *                        #addElement(JavaScriptObject, JavaScriptObject)}.
	 * @param rCallback
	 */
	private void getCompletions(final String		   sEditorText,
								final int			   nLine,
								final int			   nColumn,
								final int			   nIndex,
		...
		rAutoCompletionHandler.getCompletions(sEditorText,
			...
			new AutoCompletionCallback()
			{
				...
				public void completionsReady(AutoCompletionResult rResult)
				{
					...
					doAutoCompleteCallback(rCallback,
										   rCompletionList,
										   nFromLine,
										   nFromColumn);
				}
	...
	private native JavaScriptObject markText(JavaScriptObject rCodeMirror,
											 JavaScriptObject rStart,
											 JavaScriptObject rEnd,
	 ...
	 */
	private native void setEditorPosition(
		JavaScriptObject rCodeMirror,
		JavaScriptObject rPosition) /*-{
	 ...
	 */
	private native void setLineWrapping(
		JavaScriptObject rCodeMirror,
		boolean			 bWrap) /*-{
	 ...
	 */
	private native void setReadOnly(
		JavaScriptObject rCodeMirror,
		boolean			 bReadOnly) /*-{
	...
	private native JavaScriptObject setup(GwtCodeMirror    rGwtCodeMirror,
										  String		   sId,
										  JavaScriptObject rOptions) /*-{
		// We install an instance of the CodeMirror editor by assigning an id to
		// the intended parent element and then asking code mirror to create the
	...
	//													 }
		...
		aCodeMirror.on("change", function () {
			$entry(rGwtCodeMirror.@de.esoco.ewt.impl.gwt.code.GwtCodeMirror::fireValueChangeEvent()());
		});
		return aCodeMirror;
